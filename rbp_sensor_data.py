...
import rbp_sensor_funcs_switchbot as rbps_sb
import rbp_sensor_funcs_mhz19 as rbps_mhz19
...
def main():
    ...
    parser.add_argument("--th", action="store_true", dest="temphumi", help="Get temperature and humidity values from SwitchBot sensor")
    ...
    parser.add_argument("--plot_th", dest="plot_temphumi", action='store_true', help="Plot temperature and humidity values from SwitchBot sensor and store somewhere...")
    parser.add_argument("-p", action='store_true', help="Print out values on terminal")
    args = parser.parse_args()

    if args.co2:
        co2_and_date = rbps_mhz19.getCO2val()
        if( co2_and_date ):
            # Storage with debug mode on since I intend to call it on a cron-job and want to keep a log
            res = rbps_mhz19.addCO2valEntry( co2_and_date, True )
        ...
        # get and store!
        rbps_sb.getTempAndHumidity_SwitchBot( debugprint = True )
    elif args.plot_temphumi:
        #plot a scatter graph of the CO2 values from a Json file
        plotres = rbps_pltdata.plotTempHumidity_scatter(data_fname=rbps_io.switchbot_temphumi_log, plot_save_path=rbps_io.graphs_path_switchbot_temphumi)
        print('Plot (temp/humi) result was: {}   ({})'.format(plotres, dt.datetime.now().strftime('%Y-%m-%d %H:%M')))
    elif args.plot_co2:
        #plot a scatter graph of the CO2 values from a Json file
        plotres = rbps_pltdata.plotCO2_scatter(data_fname=rbps_io.mhz19b_co2_log, plot_save_path=rbps_io.graphs_path_mhz19b_co2)
        print('Plot (co2) result was: {}   ({})'.format(plotres, dt.datetime.now().strftime('%Y-%m-%d %H:%M')))
    elif args.plot_co2_date:
        plotres = rbps_pltdata.plotCO2_scatter(data_fname=rbps_io.mhz19b_co2_log, date_key=args.plot_co2_date, plot_save_path=rbps_io.graphs_path_mhz19b_co2)
        print('Plot (c02 date) result was: {}   ({})'.format(plotres, dt.datetime.now().strftime('%Y-%m-%d %H:%M')))
    elif args.p:
        co2_and_date = rbps_mhz19.getCO2val()
...
# will erase the lines below later
